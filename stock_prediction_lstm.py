...
def train_and_predict_gru(ticker, data, X, y, save_dir, n_steps=60, num_epochs=500, batch_size=32, learning_rate=0.001):
    ...
    with tqdm(total=num_epochs, desc=f"Training {ticker} with GRU", unit="epoch") as pbar:
...
def predict(ticker_name, stock_data, stock_features, save_dir, model_type='LSTM', epochs=500, batch_size=32, learning_rate=0.001):
    ...
    print(f"\nProcessing {ticker_name} with {model_type}")
    data = stock_data
    X, y = stock_features
    ...
    metrics_df.to_csv(os.path.join(save_dir, 'output', f'{ticker_name}_prediction_metrics_{model_type}.csv'))
    ...
    # 保存汇总报告（与 LSTM 相同）
    with open(os.path.join(save_dir, 'output', f'{ticker_name}_prediction_summary_{model_type}.txt'), 'w') as f:
